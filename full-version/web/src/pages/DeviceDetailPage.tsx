...
export default function DeviceDetailPage() {
  ...
  const [isInitialModeSet, setIsInitialModeSet] = useState(false);
  ...
  // 当设备数据加载后，初始化mode状态
  useEffect(() => {
    if (currentDevice && currentDevice.mode && !isInitialModeSet) {
      setMode(currentDevice.mode);
      setIsInitialModeSet(true);
    }
  }, [currentDevice, isInitialModeSet]);
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-[var(--color-border)] text-center">
          ...
          <Link to="/" className="text-[var(--color-primary)] hover:underline mt-2 inline-block focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] rounded">
            返回首页
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
      ...
      <header className="flex items-center gap-4">
        <button
          onClick={() => navigate(-1)}
          className="p-2 rounded-xl hover:bg-[var(--color-bg-tertiary)] transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
          aria-label="返回上一页"
        >
      ...
      {/* 设备信息卡片 */}
      <section className="bg-white rounded-2xl p-6 shadow-sm border border-[var(--color-border)]" aria-labelledby="device-info-title">
        <div className="flex items-center justify-between mb-6">
          <h2 id="device-info-title" className="text-lg font-semibold text-[var(--color-text-primary)]">
            设备信息
          </h2>
          <button
            onClick={() => loadDevice()}
      ...
      {/* 设备状态卡片 */}
      <section className="bg-white rounded-2xl p-6 shadow-sm border border-[var(--color-border)]" aria-labelledby="device-status-title">
        <h2 id="device-status-title" className="text-lg font-semibold text-[var(--color-text-primary)] mb-6">
          设备状态
        </h2>
        ...
        <div className="mb-6">
          ...
          <div
            ...
            role="progressbar"
            aria-valuenow={currentDevice.battery}
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`电量 ${currentDevice.battery}%`}
          >
            <div
        ...
        <div>
          ...
          <div
            className="h-3 bg-[var(--color-bg-tertiary)] rounded-full overflow-hidden"
            role="progressbar"
            aria-valuenow={currentDevice.signalStrength}
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`信号强度 ${currentDevice.signalStrength}%`}
          >
            <div
      ...
      {/* 设备操作卡片 */}
      <section className="bg-white rounded-2xl p-6 shadow-sm border border-[var(--color-border)]" aria-labelledby="device-actions-title">
        <h2 id="device-actions-title" className="text-lg font-semibold text-[var(--color-text-primary)] mb-6">
          设备操作
        </h2>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-[var(--color-bg-secondary)] rounded-xl">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Activity className="w-5 h-5 text-[var(--color-primary)]" aria-hidden="true" />
                <span className="text-sm font-medium text-[var(--color-text-primary)]">设置工作模式</span>
              </div>
            </div>
            <div className="flex gap-2 flex-wrap" role="group" aria-label="工作模式选择">
              {[
                ...
                <button
                  ...
                  className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-medium transition-all cursor-pointer focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] min-w-[100px] ${
                    mode === option.value
          ...
          <div className="grid grid-cols-2 gap-4">
            <Link
              ...
              className="flex items-center gap-3 p-4 bg-[var(--color-bg-secondary)] rounded-xl hover:bg-[var(--color-bg-tertiary)] transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]"
              aria-label="查看位置"
            >
              ...
              <ChevronRight className="w-4 h-4 text-[var(--color-text-muted)] ml-auto" aria-hidden="true" />
            </Link>
            <Link
              ...
              aria-label="电子围栏"
            >
              <ShieldAlert className="w-5 h-5 text-[var(--color-success)]" aria-hidden="true" />
...
function InfoItem({
  ...
  return (
    <div className="flex items-start gap-3 p-3 bg-[var(--color-bg-secondary)] rounded-xl">
      <Icon className="w-5 h-5 text-[var(--color-text-muted)] mt-0.5" aria-hidden="true" />
      <div>
...
function StatusCard({
  ...
  return (
    <div className="p-4 bg-[var(--color-bg-secondary)] rounded-xl">
      <Icon className={`w-5 h-5 ${statusColors[status]} mb-2`} aria-hidden="true" />
      <p className="text-xs text-[var(--color-text-muted)]">{label}</p>
