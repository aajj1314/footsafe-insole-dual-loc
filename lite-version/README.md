...
│  │   (3306)     │    │ (16379:6379) │    │   (8086)     │ │
│  └──────────────┘    └──────────────┘    └──────────────┘ │
│                                                              │
│  ┌──────────────────────────────────────────────────────┐   │
│  │     Terminal Service (UDP 8888 / TCP 8889 / HTTP 8091)│   │
│  │     设备协议服务 - 与智能鞋等硬件设备通信               │   │
│  └──────────────────────────────────────────────────────┘   │
│                                                              │
└─────────────────────────────────────────────────────────────┘
...
| 服务 | 地址 | 说明 |
|------|------|------|
| **前端** | http://服务器IP:8000 | Web管理后台（Nginx托管） |
| **API** | http://服务器IP:8000/api | REST API（通过Nginx代理） |
| **API直连** | http://服务器IP:8090 | REST API（直接访问） |
| **Terminal HTTP** | http://服务器IP:8091 | 设备协议HTTP接口 |
...
├── Dockerfile                 # Terminal服务Dockerfile
├── nginx.conf                 # Nginx配置
├── requirements.txt           # Python依赖
├── start.sh                   # 一键启动脚本
├── start-backend.sh           # 后端启动
...
- **端口**: 8090
- **技术**: FastAPI + Uvicorn
- **作用**: 提供REST API，管理用户、设备、报警、围栏等业务逻辑

### Terminal服务 (terminal-service)
- **端口**: 8888 (UDP), 8889 (TCP), 8091 (HTTP)
- **技术**: Python异步服务
- **作用**: 与智能鞋等硬件设备通信，接收设备上报数据，下发控制命令
- **注意**: Web功能不需要此服务正常运行，但设备接入需要

### 前端服务 (terminal-frontend)
- **端口**: 8000
- **技术**: Nginx + React构建产物
- **作用**: Web管理后台，通过Nginx代理访问API
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v2.0 | 2026-04-24 | Docker一键部署、UI/UX重构 |
